...
for F in ${NT_FILES}; do
  python3 ~/immc/scripts/clean_imgtdb.py ${F} ${OUTDIR}/fasta/${F}
	~/bin/makeblastdb -parse_seqids -dbtype nucl -in ${OUTDIR}/fasta/${F} \
...
for F in ${AA_FILES}; do
	python3 ~/immc/scripts/clean_imgtdb.py ${F} ${OUTDIR}/fasta/${F}
	~/bin/makeblastdb -parse_seqids -dbtype prot -in ${OUTDIR}/fasta/${F} \
        -out ${OUTDIR}/database/${F%%.*}
done
